...
class CurrencyListViewController: UIViewController {
    ...
    func observeOnData() {
        viewModel.loading.asObservable().subscribe(onNext: {
            ...
            if loading {
                ...
                ProgressViewHelper.dismissProgressBar()
            }
    ...
    @IBAction private func didPressChangeBaseCurrency(_ sender: UIButton) {
        ...
        McPicker.show(data: pickerData) { [weak self] (selections: [Int: String]) -> Void in
            print(selections)
            if let selectedCurrencyName = selections[0] {
                let index = pickerData[0].firstIndex(of: selectedCurrencyName)!
                let currency = CurrencyListItemViewModel(rate: CurrencyRate(currencyName: selectedCurrencyName, currencyRate: self?.viewModel.item(at: index).currencyRate ?? 0))
